...
`SIDE` / `RESET` 可以直接跳到固定视角，另有按钮切换透视/正交投影、元素编号显示和 `LVL` 响度显示。场景一次最多画 20 个
...
−10 dB 也不会横跳；低于静音底的铭牌自动淡出。**编号始终印在方块的六个面上，不随 `LVL` 移动**——
身份走深度缓冲，读数走屏幕，各有各的位置。轨迹上的面包屑则按**当时**测到的响度定尺寸。持续约两秒没有信号的对象会淡出场景——铭牌完全
消失，方块退成幽灵，但**地面的 gain 环留着**，因为"有增益却没信号"本身就是持续静音的，
把环一起藏掉就等于藏掉了最该看见的那个故障。声音一回来立即恢复。场景上方的统计条会多出一个
灰色的 Silent 计数，告诉你有几个对象已经淡出。
