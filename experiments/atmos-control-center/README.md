...
2026-09-07，根据用户最新反馈更正：**控制中心的「杜比全景声」标签仅在 7.1.4 输出时
生效，9.1.6 和 22.2 不生效**。早期实验将 9.1.6 记录为有效，该结论已撤回；以下保留
历史运行信息，并标明更正。
...
| PCM 布局 | 声道数 | ASBR 输入布局标签 | 历史观察及结论更正 |
| --- | ---: | --- | --- |
| 7.1.4 | 12 | `Atmos_7_1_4` / `0x00c0000c` | 用户确认：杜比全景声 |
| 9.1.6 | 16 | `Atmos_9_1_6` / `0x00c10010` | 早期记录为杜比全景声；2026-09-07 更正：标签不生效 |
...
上述历史记录中，当前仅保留 7.1.4 的 Atmos 标签生效结论。历史实验未进行停止辅助
AVPlayer、跨进程对照或其他设备的复核，因而不将本结果扩大为对系统内部分类机制或
普遍稳定性的证明。
