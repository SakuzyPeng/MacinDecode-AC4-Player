...
Audio settings 的两个文件选择器分别默认打开各自的文件夹，外部选择的文件复制进去之后再交给对应的异步切换机制：SOFA 走 HRTF 热切换，补偿曲线走 `adm_scene_output_set_hptf_ex`。已有的外部路径继续有效。扫描本身不证明格式合法——`hptf/` 里任何 `.txt` 都会被列出——真实解析由 MacinRender 在调用线程同步完成，失败沿用现有输出设置恢复行为。缺失条目保留在索引中，不自动替换用户选择。
