...
连接栏和退出拦截里的 **End calibration** 在会话就绪且没有写入待完成时可用，单看阶段却会误判为忙。
会话失败后保留的 `Run` 只用于收尾和展示，不再接收会话内命令；首次 **End calibration** 就先回收失败任务，
再作为独立操作重新连接原设备，不让旧任务的错误吞掉这次恢复请求。

会话写入从 `send` 成功入队起就置 `session_write_pending`，不等后台发布 `Running`。期间拒收另一条写入，
仍可显式关闭会话；退出拦截也检查这一标记，所以开始、结束和 SAVE 从排队到完成都受到保护。只由对应请求的
拒收或操作结果清除，旧请求的收尾不能解除新写入的保护。写入结束仍保留磁校准锁存；只有回读验证成功的
`mag_stop` 能清除锁存。没有待完成写入、也没有锁存的只读会话不拦截退出。
...
- 手册里的两张图 `magnetic-grid.svg`、`magnetic-turn.svg` 由 `scripts/generate-readme-diagrams.py` 生成，
  网格的列数行数、`FULL_AT`、标记尺寸和线色都转录在它顶部的一个块里；这里改了，那个块要跟着改。两张图画的是
  同一时刻，目标格与指示按 `coverage::towards` 核对过：图里的指示若与目标对不上，教的恰好是反的。
