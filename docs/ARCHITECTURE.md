...
**增益光晕**：增益是对象除位置之外携带的第二个量，此前只有两档——`OBJECT_SILENT_GAIN` 之上给
`ACCENT`、之下向 `STAGE` 褪一档——于是 −35 dB 和 0 dB 画出来完全一样。现在方块外面套一圈轮廓线，
边长在 `HALO_MIN_SCALE`~`HALO_MAX_SCALE` 之间**按分贝**取（线性读法会把下面整整 30 dB 全挤在地板
附近分辨不出），超过单位增益夹住，免得热对象长到邻居身上。方块本身保持定尺寸：它是光晕被读数的
参照系，而且面上的编号需要一个稳定的尺寸才读得清。

- **地板复用 `OBJECT_SILENT_GAIN`，不另立常量。** mockup 表里的 −36 dB 光晕地板和这个常量本来就是
  同一个数。共用之后「光晕触底」和「方块褪成静默色」由构造发生在同一个增益上——一个阈值两个通道，
  日后不可能各自漂走；有一条测试专门钉这件事。
- **只画轮廓，这是正确性不是风格。** 正交模式下整条投影链是仿射的，所以同心放大 k 倍的盒子，其投影
  恰好是方块投影绕同一中心放大 k 倍。完整线框的近端角因此仍落在方块轮廓内部，从那里辐射的三条棱会
  直接划过三个可见面——正是编号所在的地方。轮廓按定义是投影的边界，而凸区域绕内点放大严格包含自身，
  于是光晕永远碰不到编号。
- **轮廓棱的判定必须写成两个朝向布尔量的异或**，不能写成符号乘积。两者在一般视角下等价（各轴 2 条、
  共 6 条），但轴对齐时乘积形式每一项都带一个为零的分量，严格小于零永不成立，**12 条棱一条都不画**
  ——光晕会在 TOP/BACK/SIDE 三个预设上整个消失。异或把「恰好侧对」算作背面，轴对齐时正好剩下可见
  那一面的 4 条边界棱。`mesh.rs` 有一条测试覆盖轴对齐这一档。
- **LFE 不给光晕**，和垂线、轨迹一样：三套系统的缺席就是「无位置」的语义信号，光晕因此又多一条把
  有位置的动态对象和无位置的 LFE 分开的线索。面包屑也不给——理由与轨迹不带增益着色的那条相同。
