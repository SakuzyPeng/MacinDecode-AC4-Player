...
**一个没有解析出动态对象或 LFE 状态的 quantum 会被丢弃而不是存成空场景**——前向时间线空隙和 FIFO
...
镜像每个动态对象槽位另外保存一圈 **40 × 10 ms = 400 ms** 的 K 加权能量 bin（`LoudnessBin`：平方和、帧数、
样本峰值）。400 ms 正是 ITU-R BS.1770 的 momentary 窗口，所以把整圈求和得到的就是标准定义的那个量
本身，而不是它的近似。

LFE 使用独立的 `LfeView` 和 `LFE_METER_SLOT`，不进入动态对象数组或参考系计数。
三个消费者均采集未加权 LFE PCM 平方和、帧数及样本峰值，计入有效元数据增益，排除主音量。
MacinRender 将 LFE 能量保留在元数据历史中，按呈现位置发布；纯 LFE 场景也能发布镜像。
LFE 的 bin 和显示侧峰值在换 epoch、元素身份变化或通道移除时清空。
...
`SceneViewFrame::sample_peak` 点亮的 clip。**动态对象的 clip 刻意不过 K 加权**——转换器不是按
听觉模型削顶的。
...
LFE 在表带中显示为第 0 行，在前墙柜体上显示铭牌，始终使用未加权 30 ms RMS dBFS。
它拥有相同的增益刻线、峰值保持及削波提示，但不套用 LUFS-M：BS.1770 的节目响度排除 LFE。
因此总计量容量为 20 个动态对象加一条 LFE，不挤占动态对象的可见数量，也不为 LFE 创建位置或轨迹。
