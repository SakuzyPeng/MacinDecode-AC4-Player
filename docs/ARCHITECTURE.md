...
三个消费者均以独立的 K 加权滤波器采集 LFE 能量、帧数及未加权样本峰值，计入有效元数据增益，排除主音量。
...
`MeterReadout` 是全界面唯一的计量选择：表带所有行、动态对象和 LFE 铭牌、足迹内芯及轨迹强弱
...
UI 按所选单位显示单声道诊断读数，并由 `backend::state` 的对拍测试（dev-dependency `ebur128`）保证它和参考实现
在 0.01 LU 内一致。
