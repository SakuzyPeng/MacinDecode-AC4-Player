...
就用它，没有就用该模式的第一页。回退只决定当前显示，只有显式点击页签才更新记忆值（包括点击
已经显示的回退页）。因此绕到别的模式再回来，原来那一页还在。初值取 `Speakers`
...
的标记。
导入失败或工作线程未能启动时也会清理暂存目录和保存标记，保留原 profile 与旋钮值，避免下一次普通导入
被误判为保存完成而归零旋钮。
...
seek、单曲重播及切歌在输入采样率、输出设备、模式和布局兼容时复用同一原生会话与 HRTF。
切歌的 Opening 阶段只暂停旧输出并停止旧 reader；新 Scene 就绪后由同一 producer 递增原生 epoch，
清空设备、卷积／HpTF 历史、元数据及响度历史，再配置新对象集合。对象 ID、数量与 LFE 可以变化。
待处理的源替换仅保留最新请求；请求与快照发布共用序列屏障，旧轮次的 EOS 和位置不会覆盖新目标。
重置失败时允许一次完整重建；输出格式或设备不兼容、解码关闭等情况仍释放原会话。
...
方向插值表另设共享只读缓存（最多八组、16 MiB），以有序方向坐标的浮点位模式识别几何，
不缓存音频历史。同一几何的并发准备只构建一次；HRIR 或采样率改变仍重新读取、转换频域数据。
因此完整频域数据超过 64 MiB 的大 SOFA 也能复用约 1.5 MiB 的插值表。
冷构建使用边邻接的三维 hull 和固定查询网格的包围盒树，直接生成稀疏权重，保留全部方向、
1° 网格、原有三角形优先顺序和归一化规则；没有降低 HRTF 精度或依赖磁盘预热。
...
再读取 Euler 或原生四元数；检查结果为 Complete 后，设备线程仍须等待该任务收尾再配置并启动采集。
寄存器四元数为显式备用选项。首次安装轴必须由用户选择。
...
安装轴检查不访问设备，只读已呈现的姿态。一个物理动作绕单一头部轴旋转——点头绕 right、摇头绕 up、
侧倾绕 forward——而 mounting 正是把传感器轴映射到这三个头部方向，所以动作在姿态上转出的规范轴，
指出当前哪一项存着该动作真正使用的传感器轴，带符号取出即为该动作对应项的正确值；三个动作定出全部三项。
相对旋转取自动作起始姿态所在的机体系，结果不随听者当时朝向改变。低于 20°、主轴占比低于 0.85、
两项落在同一轴或三轴构成左手系一律拒绝，不取平均，也不自动写入；界面给出映射，由用户应用并重新连接。
检查结果绑定到实际连接的设备配置、worker request 与传感器 instance/session/reference epoch；
这些身份改变、停止追踪或切换来源时清除整组结果。当前设置与实际连接配置不一致时暂停检查，
避免应用映射后、重新连接前把旧姿态按新映射解释。重做动作在开始时清除该项，失败或取消不复用旧观测。
纯算术在 `src/posebridge/mounting.rs`，不受 `posebridge_input` 门控，测试在所有平台随 `cargo test` 运行。

设备操作结果保留发送/回读/观察到完成/持久化状态。设备角度参考自带 SAVE，恢复默认也保存；
其他操作不附加 SAVE。磁校准未明确结束时保留退出保护，取消写入不代表回滚。
退出检测与拦截在 `App::logic` 执行，窗口最小化或被遮挡时仍有效；需要处理设备操作时恢复窗口，
对话框只在 `App::ui` 绘制。等待设备操作结束期间，逻辑层持续请求更新。
