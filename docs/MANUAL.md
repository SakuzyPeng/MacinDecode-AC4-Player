...
动态对象响度是渲染前测的，按 ITU-R BS.1770 的 K 加权计算，并有对拍测试保证它和参考实现一致。
需要说明的是，逐对象响度不是一个标准量——BS.1770 定义在基于声道的节目上，对象音频要先渲染到参考布局
才能测。所以场景里这个读数标的是 dBFS，而不是假装它是别的什么。

**0 号 LFE 也有电平铭牌**，固定在前墙柜体上方。它显示未加权 RMS dBFS 电平，计入元数据增益、
不受主音量影响；LFE 不计入 BS.1770 节目响度，因此不套用动态对象的 K 加权或 LUFS 标尺。
...
存在 LFE 时，最上方增加 **0 号**一行，包含电平、增益刻线、峰值保持和削波指示。表头的 **0: dBFS**
说明它始终使用 dBFS，即使动态对象切换到 LUFS-M。所有行的电平条和数值列保持对齐；LFE 不占用
20 个动态对象的显示额度。
