...
在 **Device** 页先用底部 **Read settings** 读取设备配置，再编辑右侧的采样率、格式和融合模式。
控件中的**绿色 ✓** 标记设备回读值，**橙色 •** 标记选中的待应用值，不另设重复的当前值对照区。
可以连接时编辑这些草稿；写入前用顶部 **Disconnect** 停止采集，再点击底部统一的 **Apply changes**。
播放器先重新读取配置，仅写入本次改动，并逐项回读、最后再次核验整组配置。外部配置已变化、取消或部分失败
都会明确报告，不会把未完成的更改标成成功；**Discard edits** 只撤销草稿，不回滚已经写入的设备。

**Prepare enhanced data** 将合适的输出格式填入同一份草稿，保留所选速率；仍通过 **Apply changes** 写入。
当前选择已验证的 **0xA4 时间戳＋角速度＋四元数**。30 字节完整惯性帧 **0xE4** 仅为 20 Hz 实验配置，
USB／BLE 三轴实机门槛尚未完成，因此不开放正式预设。已有 Motion 格式的加速度仍显示在诊断中，
但没有时间戳时按普通模式运行。完成后手动 Connect，常规配置应用不执行校准、归零或 Flash 保存。
...
校准、归零、Flash 保存及恢复默认单独放在 **Maintenance** 页，按“校准与参考方向”和“保存设置”分栏。
这些操作保留就地弹出的确认框，不混入常规 **Apply changes**；会写入已保存设置的操作以警示色标出。
操作结束后手动 Connect。设备归零要求六轴模式；角度参考命令包含 SAVE，恢复默认也保存。
磁校准不在这里开始：**Magnetic calibration…** 通往它自己的一页（见下文），校准期间一直读着磁场。
