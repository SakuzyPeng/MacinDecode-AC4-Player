...
nothing before it, and must never be presented as a decode result — `DecodeContainer::Generated`
labels itself `built-in demo` for exactly that reason.
...
The demo needs none of that either, which is why `decoder::worker::tests::plays_the_built_in_demo_without_media_or_an_index`
and `backend::preview::tests::the_demo_walks_into_the_scene_view` are the first tests to cover the
worker's command path and the preview consumer in a plain `cargo test`. To hear the demo,
`MACINDECODE_DEMO_WAV=/tmp/demo.wav cargo test decoder::demo::tests::folds_the_demo_down_for_listening -- --ignored`
writes a stereo excerpt.
...
The demo's 16 object slots hold fixed element IDs for the whole programme, so `SceneSignature` never
changes and no phase boundary can force a rebuild. Its score is generated by
`scripts/generate-demo-score.py` and committed; re-run it after changing the tempo, the phase map or
the sources, and `--check` fails if `src/decoder/demo/score.rs` is stale. `docs/ARCHITECTURE.md`
carries the contract.

Demo transport has its own mode (Repeat one by default, or Play once). It reuses ordinary epoch-based
seek/replay, including an exact seek to EOS. The file session stays parked in the checkpoint while
demo timing advances; stopping the demo restores that session paused. The app's demo integration
tests exercise this with the silent native output or scene preview, without media fixtures.

Synthesis windows each note before mixing. Each generated block resolves its starting metadata from
absolute time and carries unfinished gain ramps explicitly; position updates never reset gain ramps.
...
- **`feature = "decode"`** — there is a decoder. `decoder/worker.rs`, the parts of `decoder.rs` that
  drive it, `backend/preview.rs`, and `decoder::demo`'s synthesiser (`voice.rs`, `stage.rs`,
  `DemoProgram`), which builds Scene blocks whose constructors this gate owns. `demo/score.rs` is
  plain data and stays ungated, so the inspection-only shell still compiles it. Nothing here is
  platform-specific: `decoder/worker.rs` imports only `std` and the Core crates and calls no OS API
  at all.
...
- **`posebridge_input`** = macOS/Windows + `posebridge`. Device worker and UI; saved preferences remain
  portable without this gate. `posebridge::consumption`, `posebridge::mounting` and `posebridge::coverage`
